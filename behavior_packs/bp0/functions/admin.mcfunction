# Make the executing player an admin observer
# Usage: /function admin
# @s = player executing the function
...
tellraw @s {"rawtext":[{"text":"§6관리자 관찰자가 되었습니다. 자동으로 팀 0에 배정되고 크리에이티브 모드로 관찰할 수 있습니다."}]}
tellraw @s {"rawtext":[{"text":"§7Observer system modified by stevecoding"}]}
say §e[관리자] §6플레이어가 관리자 관찰자 권한을 받았습니다.